...
// Description: An example that tests the raw acquisition speed测试原始采集速度
// Acquisition mode used is triggered streaming with internal trigger内部触发  触发流  
//
...
int main(int argc, char* argv[])
{
	...
	{
		for (tmp_adq_num = 0; tmp_adq_num < n_of_devices; tmp_adq_num++)//打印所有设备信息
		{
	...
	if (ADQControlUnit_OpenDeviceInterface(adq_cu, adq_num))//用第adq_num个设备
		printf("success!\n");
	else
	{
		printf("failed!\n");
	...
	if (ADQControlUnit_SetupDevice(adq_cu, adq_num))//初始化该设备
		printf("success!\n");
	else
	{
		printf("failed!\n");
	...
	n_of_ADQ = ADQControlUnit_NofADQ(adq_cu);//获取ADQs的数量
	...
	n_of_opened_devices = ADQControlUnit_NofADQ(adq_cu);//获取ADQs的数量
