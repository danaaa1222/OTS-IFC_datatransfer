...
void* consumeItem(int num)
{
	while (1)
	{
		nodes tmp;
		q.wait_dequeue(tmp);
		write_record_to_file(tmp.data_address, tmp.data_bytes / 2, tmp.file_number);//-写细胞数据
				...
				for (ch = 0; ch < 4; ++ch)
				{
					...
					if (samples_added[ch] > 0)
					{
						...
						if (samples_extradata[ch] > 0)
						{
							...
							{
								//拼完整交界处的不完整细胞
								memcpy(alt_buffer, target_buffers_extradata[ch], sizeof(short) * samples_extradata[ch]);
								//计算剩下的那个细胞的长度
								...
								memcpy((alt_buffer + samples_extradata[ch]), target_buffers[ch],
									sizeof(short) * (samples_added[ch] - samples_remaining));

								memcpy(alt_header, target_headers[ch], sizeof(struct ADQRecordHeader) * headers_done);
								//	for (int kk = 0; kk < headers_done; kk++)
									//	alt_header[kk] = target_headers[ch][kk];
								alt_node.data_address = alt_buffer;
								alt_node.data_bytes = sizeof(short) * (samples_added[ch] - samples_remaining + samples_extradata[ch]);
								alt_node.file_number = count_record;
								alt_node.head_address = alt_header;
						...
						else
						{
							...
							else
							{
								...
								//if (alt_buffer && alt_header)
								//计算不完整的那个细胞的长度
								for (int i = 0; i < headers_done; ++i)
									samples_remaining -= target_headers[ch][i].RecordLength;
								//把完整的给alt_buf\alt_header
								memcpy(alt_buffer, target_buffers[ch], sizeof(short) * (samples_added[ch] - samples_remaining));
								memcpy(alt_header, target_headers[ch], sizeof(struct ADQRecordHeader) * headers_done);
								//	for (int kk = 0; kk < headers_done; kk++)
									//	alt_header[kk] = target_headers[ch][kk];
								alt_node.data_address = alt_buffer;
								alt_node.data_bytes = sizeof(short) * (samples_added[ch] - samples_remaining);
								alt_node.file_number = count_record;
								alt_node.head_address = alt_header;
								alt_node.header_number = headers_done;
								q.enqueue(alt_node);
						...
						{
							memcpy(target_headers[ch], &target_headers[ch][headers_done], sizeof(struct ADQRecordHeader));
							memcpy(target_buffers_extradata[ch],
