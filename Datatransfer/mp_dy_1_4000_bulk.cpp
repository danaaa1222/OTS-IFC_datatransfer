...
/*-----------------------------------------------------------------------------------
	...
	修改（1）：线程结束不了 、 有野指针【已全部修复】
	修改（2）：从队列中批量出 wait_dequeue_bulk_timed
			参数：consume  40 每次最多出列的数量
	修改（3）：5个消费者线程+1个生产者线程
-----------------------------------------------------------------------------------------*/
...
void* consumeItem1(int num)
{
	...
	{
		nodes tmp[consume];
		int counttt = q.wait_dequeue_bulk_timed(tmp, consume, 3000);
		//printf("counttt %d \n", counttt);
		for (int jjj = 0; jjj < counttt; jjj++)
			if (mem_flag[tmp[jjj].mem_pool_location] == 1)
			{
				write_header_record_to_file(tmp[jjj].head_address, tmp[jjj].header_number,
					tmp[jjj].data_address, tmp[jjj].data_bytes / 2, tmp[jjj].file_number);//-写细胞数据
				free(tmp[jjj].head_address); tmp[jjj].head_address = NULL;

				mem_flag[tmp[jjj].mem_pool_location] = 0;
			}
	}
	popOver = 0;
	printf("consumeItem1 end\n");
	return((void*)0);
}
...
void adq_transfer_test(void* adq_cu, int adq_num, int adq_type)
{
	...
	t1.join();
	//printf("t1.join() end\n");
	t2.join();	//printf("t2.join() end\n");
	t3.join();		//printf("t3.join()end \n"); 
...
void adq_perform_transfer_test(void* adq_cu, int adq_num, int adq_type) {
	...
	do {
		...
		//printf("W");		//获取可用的DMA buffer的数量。
		CHECKADQ(ADQ_GetTransferBufferStatus(adq_cu, adq_num, &buffers_empty));
		...
		//buffers_empty = 0：no DMA buffer  is ready to be read意思是DMA里面没有足够的数据
		while (!buffers_empty)
		{
			// Mark the loop start
			timer_start(2);
			...
			if (!buffers_empty) {
				printf("\n[NOTE]   Timeout, flushing DMA...no DMA buffer  is ready to be read\n");
				CHECKADQ(ADQ_FlushDMA(adq_cu, adq_num));
			}
		}

		//If buffers_empty = sel_tr_buf_no
		// all buffers are full, and an overflow will likely occur soon
		if (buffers_empty >= (sel_tr_buf_no - 1))
		{
			printf("\n[WARNING] Maximum buffer fill level detected.all buffers are full, and an overflow will likely occur soon\n");
		}
		else if ((sel_tr_buf_no > 4) && (buffers_empty >= (sel_tr_buf_no - 3)))
		{
			printf("\n[WARNING] High buffer fill level detected.\n");
		}

		if (buffers_empty > max_used_tr_buf)
			max_used_tr_buf = buffers_empty;




		while (buffers_empty > 0)
		{
			while (memory_pool_num < tgtbuf_num)
			{
				if (mem_flag[memory_pool_num] == 0)
				{
					...
					target_headers[0] = (struct ADQRecordHeader*)malloc(target_header_malloc_byte);
					...
					received_all_records = (records_completed[0] >= nof_records_sum);//nof_records_sum=-1时，实际的值是4*1024*1024*1024-1 32位  10^5cell/s 10万细胞 1000s之后才需要考虑这个条件
...
error:
	...
	printf(" Total number of records received    = %12llu", records_completed[0]);
	if (parse_mode == 1)
		printf("\n");
	else
		printf(" (N/A for RAW/no-parse modes).\n");
