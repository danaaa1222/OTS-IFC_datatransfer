...
/*-----------------------------------------------------------------------------------
	对比mp_dy_1_4000_bulk代码：
		改了内存池分配512个；
		去掉了消费者线程flag=1的判断；
		速度能到4000MB；
		参数：
			trig_frequency = 8400*2;
			trig_period = 10000000000 / trig_frequency;
			samples_per_record = 1024*128;
			int max_run_time = 30;
			unsigned int sel_tr_buf_no = 170;
			unsigned int sel_tr_buf_size = 2 * 1024 * 1024 * sizeof(short);

			12/27
				去掉了生产者的if (headers_added[ch] > 0)
-----------------------------------------------------------------------------------------*/
...
#define tgtbuf_num 512/4   //开局分配  tgtbuf_num 个targetbuffer_size 大小的内存
short* mem_address[tgtbuf_num];
int mem_flag[tgtbuf_num];//0可用，1不可用
#define consume  4

int bulk_order = 0;
#define num_bulk_queue 32
...
//int buffers_filled_alt = 0;
//int mmm[4];
void* consumeItem1(int ID)
{
	nodes tmp[consume];
	//mmm[ID] = 0;
	while (popOver)
	{
		...
		//printf("consumeItem %d size_approx %d \n", ID, q.size_approx());
		int counttt = q.wait_dequeue_bulk_timed(tmp, consume, 3000);
		//int counttt = q.try_dequeue_bulk(tmp, consume);
		//printf("consumeItem %d buffers_filled_alt %d memory_pool_used %d  counttt %d\n", ID, buffers_filled_alt, memory_pool_used, counttt);
			...
			//if (mem_flag[tmp[jjj].mem_pool_location] == 1)
		{
			...
			memory_pool_used--;
			//mmm[ID]++;
			//printf("consumeItem1 counttt  %d tmp[jjj].mem_pool_location %d\n", counttt,tmp[jjj].mem_pool_location);
		}
	}
	popOver = 0;
...
void adq_perform_transfer_test(void* adq_cu, int adq_num, int adq_type) {
	...
	do {
		...
		{
			//printf("productor buffers_filled  %d  memory_pool_num %d memory_pool_used %d\n", buffers_filled, memory_pool_num, memory_pool_used);
			/*while (memory_pool_num < tgtbuf_num)
			{*/
			//if (mem_flag[memory_pool_num] == 1)
			//{
			//	printf("重复");
			//}
		//buffers_filled_alt = buffers_filled;
			...
			alt_node.data_address = alt_buffer;*/
					...
					alt_node[bulk_order].mem_pool_location = memory_pool_num;
