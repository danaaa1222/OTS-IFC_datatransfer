...
/*-----------------------------------------------------------------------------------
	功能： targetbuffer动态分配、head动态分配、【head】写文件【buffer】写文件
	说明： 每获取一次head、buffer，都写入单个文件，即head写的是小文件
	参数：
				header动态分配 10 * 1024
	速度测试：
	//		setting:
...
unsigned int mnn_thread_w[4];
unsigned int effict_time[4];
unsigned int effict_time_free[4];
void* consumeItem1(int ID)
{
	nodes tmp[consume];
	effict_time[ID] = 0;
	mnn_thread_w[ID] = 0;
	effict_time_free[ID] = 0;
	timer_start(ID * 3 + 4);
	tref[ID * 3 + 5] = 0;
	tref[ID * 3 + 6] = 0;
	while (popOver)
	{
		int counttt = q.wait_dequeue_bulk_timed(tmp, consume, 0);
		for (int jjj = 0; jjj < counttt; jjj++)
		{
			timer_start(ID * 3 + 5);	
			write_header_record_to_file(tmp[jjj].head_address, tmp[jjj].header_number, tmp[jjj].data_address, tmp[jjj].data_bytes / 2, tmp[jjj].file_number);//-写细胞数据
			//printf("%f\n", timer_time_us(ID * 3 + 5));	
			effict_time[ID] += timer_time_ms(ID * 3 + 5);
			timer_start(ID * 3 + 6);
			free(tmp[jjj].data_address);
			free(tmp[jjj].head_address);
			tmp[jjj].data_address = NULL;
			tmp[jjj].head_address = NULL;
			mnn_thread_w[ID]++;
			effict_time_free[ID] += timer_time_ms(ID * 3 + 6);
		}
	}
	popOver = 0;
	printf("consumeItem %d end mnn_thread_w[ID] %d;\n", ID, mnn_thread_w[ID]);
	printf("consumeItem %d total_time % 9d ms effict_time %d ms  write_a_file_time %d ms effict_time_free %d\n", ID, timer_time_ms(ID * 3 + 4), effict_time[ID], effict_time[ID] / mnn_thread_w[ID], effict_time_free[ID]);
	return((void*)0);
}
...
void adq_perform_transfer_test(void* adq_cu, int adq_num, int adq_type)
{
	...
	do {
		...
		while (buffers_filled > 0)
		{
			...
			target_buffers[0] = (short int*)malloc((size_t)sel_tr_buf_size);
			if (!target_buffers[0]) {
				printf("Failed to allocate memory for target_buffers\n");
				goto error;
			}
			target_headers[0] = (struct ADQRecordHeader*)malloc((size_t)10 * 1024);
			if (!target_headers[0]) {
				printf("Failed to allocate memory for target_headers\n");
				goto error;
			}
			effect_time_write += timer_time_ms(18);
	...
	{
		printf("\n\nDone.\n\n");
		printf("count_record %d fetch+malloc time is %d malloc time %d\n", count_record,timer_time_ms(1), effect_time_write);
		CHECKADQ(ADQ_GetWriteCountMax(adq_cu, adq_num, &dram_wrcnt_max));
	}
	else
		printf("\n\nError occurred.\n");
